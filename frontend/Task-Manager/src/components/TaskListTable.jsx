...
const TaskListTable = ({tableData}) => {
    ...
    const getPriorityBadgeColor = (priority) => {
        switch (priority) {
            ...
            case 'Medium': return 'bg-yellow-100 text-yellow-500 border-yellow-200';
            ...
            <tbody>
                {tableData.map((task) => (
                    <tr key={task._id} className="border-t border-gray-200 ">
                        ...
                        <td className="py-2 px-4 text-gray-700 text-nowrap hidden md:table-cell">{task.createdAt ? moment(task.createdAt).format('DD MM YYYY') : 'N/A'}
                        </td>
                    </tr>
                ))}
            </tbody>
